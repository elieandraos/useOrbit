...
`lab-it`'s Plan Synthesis track can hand off an approved, initiative-specific section of `plan.md` — architecture, locked product decisions, preserved behavior, and stated constraints, already reasoned through and approved by the user before feature planning ever starts. This file governs how `plan-it` consumes that handoff. It's an **additional input path, not a requirement**: normal conversation-driven planning remains valid whenever no matching approved plan exists.
...
`lab-it` owns producing the approved plan — investigation, target-architecture design, locking decisions with the user, writing and reviewing `plan.md`. `plan-it` owns everything downstream of an approved plan: classification, scope checklists, design reconciliation, canonical issue definitions, review, and the GitHub workflow. This rule doesn't create or amend `plan.md`, and it doesn't re-derive architecture `plan.md` already settled — that stays `lab-it`'s Plan Synthesis track.
