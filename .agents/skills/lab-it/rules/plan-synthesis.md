...
initiative-specific `plan.md` handoff for `plan-it`. The same procedure applies to a
...
`plan-it` start from settled architecture and settled product decisions instead of
...
- The **source of truth** for the subsequent feature-planning pass. Once approved,
  `plan-it` should not need to re-derive architecture or re-litigate decisions from
  this conversation.
...
- A place to classify the feature, scope it, reconcile it against design files, decompose it into
  issues, sequence delivery, or touch GitHub — all of that is `plan-it`'s job, done
...
of truth for the subsequent `plan-it` pass, and that implementation details still need
...
`plan-it/rules/plan-md-input.md`.
...
- The plan is sufficient for `plan-it` to distinguish settled from open without
...
it to `plan-it`, and don't imply its decisions are settled. After approval, the
matching initiative-specific section becomes canonical input under
`plan-it/rules/plan-md-input.md`. Don't invent a persistent approval marker or an
