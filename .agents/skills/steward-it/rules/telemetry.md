...
### Skill activation footprint vs. ongoing context reuse

When evaluating whether one skill is genuinely heavier than another, prefer measurements taken from the
skill's own activation event over cumulative per-skill totals. A useful activation-footprint comparison
can include the injected `SKILL.md` size, activation-turn output/thinking, and activation-turn cache
creation/read values when those fields are available.

Do not interpret the following as a skill's ongoing context cost:

- cumulative `cache_read` attributed to a sticky `attributionSkill` interval;
- output/thinking generated by later implementation, testing, review, or GitHub work that inherited the
  same attribution label;
- the total context/cache volume of the assembled conversation without a per-document attribution
  mechanism.

The session log does not expose a reliable per-document breakdown of cached context. It cannot establish
how many cache-read tokens belong specifically to a skill's rules, blueprints, templates, or other files
that remain available in context, nor whether later model reasoning actually relied on those materials.
Do not claim an exact ongoing carrying cost without stronger runtime evidence.

Where a skill appears disproportionately large in attributed totals, first reconstruct the activation order
and the contiguous attribution intervals. If the skill was simply the last activated skill before a long
period of ordinary work, classify the apparent excess as attribution spillover unless other evidence shows
a real footprint difference. A different activation order in a separate controlled run would be needed to
prove the exact counterfactual allocation; a single run establishes the mechanism, not that counterfactual.
...
2. **Locate the log file.** Each session is stored as a JSONL file at
   ...
   - **Stewarding a different or historical session** (for example, investigating an earlier session
     from within a later one): do not guess a session id. Search the same project directory for the JSONL
     whose content contains a known, distinguishing string (an issue number, a commit SHA, a quoted
     prompt) or whose modification time matches the known event window, and confirm the match before relying
     on it.
...
involved) is also a measurable human-wait interval — the gap between the report's timestamp and the next
user turn's timestamp.
...
records. Treat those as historical intent only — they are not runtime measurements and must not be cited
as current consumption proof.
