...
New `config/documents.php` centralizes disk name, max size (25 MB), allowed mimes (`pdf, jpg, jpeg, png, doc, docx, xls, xlsx`), and the stale-pending prune threshold (`prune_after_hours`, default 1) — so validation/storage don't hardcode the first two twice, and the prune command (below) doesn't hardcode a magic number either. Swapping to S3 later is an env var change (`config/filesystems.php` already has an `s3` disk templated but inactive).
